...
exports.run = async(client, message, args) => {
  ...
  if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send('Bu komutu kullanabilmek için "\`Üyeleri At\`" yetkisine sahip olmalısın!')//Frenzy - Krossz
 ...
 if (db.has(`log_${message.guild.id}`) === false) return message.channel.send(` Mod log kanalı ayarlanmamış ayarlamak için | ${prefix}modlog #kanal`);
  ...
  user.send(`**${message.guild.name}** Adlı Sunucuda Yaptığınız Olumsuz Davranışlardan Dolayı Atıldınız! \n Yetkilinin Girdiği Sebep: **${reason}**`)//Frenzy - Krossz

  message.guild.member(user).kick();
  
  message.channel.send(`Kullanıcı başarıyla atıldı!`)//Frenzy - Krossz
