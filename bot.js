      ...
      try {
        if (!message.member.permissions.has("BAN_MEMBERS")) {
          ...
          return message.channel
            .send(
              ` Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!`
            )
      ...
      try {
        if (!newMessage.member.permissions.has("BAN_MEMBERS")) {
          ...
          return newMessage.channel
            .send(
              ` Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!`
            )
            .then((matador) => matador.delete({ timeout: 10000 }));
        }
      } catch (err) {
      ...
      if (!message.member.hasPermission("ADMINISTRATOR")) {
        ...
        if (uyarisayisi === null) {
          message.channel.send(
            `<@${message.author.id}> Reklam Koruma Sistemine Yaklandın! **İlk Uyarın** Devam Edersen Atılacaksın! (1/3)`
          );
        }
        if (uyarisayisi === 1) {
          message.channel.send(
            `<@${message.author.id}> Reklam Koruma Sistemine Yaklandın! **2. Uyarın** Devam Edersen Atılacaksın! (2/3)`
          );
        }
        if (uyarisayisi === 2) {
          message.delete();
          ...
          message.channel.send(
            `<@${message.author.id}> **3. Reklam Uyarısı** Aldığı Tespit Edildi ve Sunucudan Atıldı! (3/3)`
          );
        }
        if (uyarisayisi === 3) {
          message.delete();
...
client.on("emojiUpdate", async (oldEmoji, newEmoji) => {
  ...
  const c = newEmoji.guild.channels.cache.get(
    db.fetch(`log_${newEmoji.guild.id}`)
  );
  if (!c) return;
...
client.on("messageUpdate", async (oldMsg, newMsg) => {
  ...
  var kanal = oldMsg.guild.channels.cache.get(
    db.fetch(`log_${oldMsg.guild.id}`).replace("<#", "").replace(">", "")
  );
  if (!kanal) return;
  const embed = new Discord.MessageEmbed()
    .setColor("BLUE")
    ...
    .setDescription(
      `<@!${oldMsg.author.id}>  adlı kullanıcı tarafından <#${oldMsg.channel.id}> kanalına gönderilen mesaj değiştirildi!\n\n**Kullanıcı:** <@!${oldMsg.author.id}>\n\n **Eski Mesaj:** ${oldMsg.content}\n\n**Yeni Mesaj:** ${newMsg.content}`
    )
    .setTimestamp()
    .setFooter(client.user.username, client.user.avatarURL())
